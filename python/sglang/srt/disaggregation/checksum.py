...
def page_indices_for_request(scheduler, req: Req, end_idx: int) -> torch.Tensor:
    ...
    # Same translation the send path applies in `_send_kv_chunk`: on a
    # unified-memory pool `req_to_token` holds VIRTUAL ids while the registered
    # buffers are addressed by physical ones, so digesting the raw ids reads
    # the wrong pages.
...
def state_indices_for_request(
    ...
    if isinstance(pool, SWAKVPool):
        ...
        # the latter returns kernel-facing ids (see unified_hybrid_swa.py:301),
        # while the SWA component is registered as page envelopes and addressed
        # by physical ids (:337). The send path uses the former.
        window_swa = (
